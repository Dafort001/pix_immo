  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-3 sm:p-4">
      ...
      <button
        ...
        className="lg:hidden flex items-center justify-between w-full text-gray-700"
      ...
      <div className={`${isOpen ? 'block' : 'hidden'} lg:flex flex-wrap gap-2 mt-3 lg:mt-0`}>
        {filters.map((filter) => (
          ...
          >
            {filter.label}
            <span className="ml-2 px-1.5 py-0.5 rounded-full bg-white/20 text-xs" data-testid={`text-count-${filter.value}`}>
              {filter.count}
            </span>
          </Button>
        ))}
