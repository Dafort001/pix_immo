...
- **Editor Job Page**: Tokenized portal for external photo editors to download RAW files and upload edited images. Features SHA-256 token hashing, expiration checks (7 days), rate limiting (20 req/min), and secure presigned URL generation for R2 operations. Supports individual JPEG upload workflow with automatic database logging. Accessed via `/editor/job/:jobId?token=<hash>` with English-only UI. Backend API provides 4 routes: GET job details, POST download URL, POST upload URL, POST upload completion. Implementation in `server/editor-job-routes.ts`, `client/src/pages/editor-job.tsx`.
...
- **Photography Package System Redesign (Nov 23, 2025)**:
  ...
  - New content services: Alt-Texte für KI-Bildsuche (€10, CSV-Export für CRM-Import), Basis-Exposé Texterstellung (€15, requires Alt-Texte)
